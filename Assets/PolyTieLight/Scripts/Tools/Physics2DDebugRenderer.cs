    ...
    private void renderDebugData()
    {
        ...
        if (_joints != null)
        {
            ...
            for (int i = 0; i < _joints.Length; i++)
            {
                ...
                if (_joints[i].GetComponent<HingeJoint>() != null)
                {
                    GL.Vertex(_joints[i].connectedBody.transform.position + new Vector3(_joints[i].GetComponent<HingeJoint>().connectedAnchor.x, _joints[i].GetComponent<HingeJoint>().connectedAnchor.y, 0));
                    GL.Vertex(_joints[i].transform.position + new Vector3(_joints[i].GetComponent<HingeJoint>().anchor.x, _joints[i].GetComponent<HingeJoint>().anchor.y, 0));
                }
            }
            GL.End();
        }
